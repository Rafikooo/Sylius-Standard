...
final class MyOrderInventoryOperator implements OrderInventoryOperatorInterface
{
    ...
    public function cancel2(OrderInterface $order): void
    {
        dd('overriden');
        if (in_array(
            $order->getPaymentState(),
    ...
    private function release(OrderInterface $order): void
    {
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            $variant->setOnHold(2138);
            $variant->setTracked(true);
            $variant->setOnHand(4096);
        }
    }
